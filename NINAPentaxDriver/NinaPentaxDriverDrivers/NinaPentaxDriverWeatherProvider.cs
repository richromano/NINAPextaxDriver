...
namespace Rtg.NINA.NinaPentaxDriver.NinaPentaxDriverDrivers {
    ...
    public class NinaPentaxDriverWeatherProvider : IEquipmentProvider<IWeatherData> {
        private IProfileService profileService;

        [ImportingConstructor]
        public NinaPentaxDriverWeatherProvider(IProfileService profileService) {
            this.profileService = profileService;
        }

        public string Name => "NinaPentaxDriver";

        public IList<IWeatherData> GetEquipment() {
            var devices = new List<IWeatherData>();
            devices.Add(new NinaPentaxDriverWeatherDriver("MyUniqueId", "MyDeviceId"));
